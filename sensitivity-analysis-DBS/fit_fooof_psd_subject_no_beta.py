...
def fit_fooof_psd_subject_no_beta(sub): #,PATH_ANALYSIS,freq_range = [1, 250]):    
    ...
    PATH_ANALYSIS = '/Users/ao622/Dropbox (Personal)/Lab-BML/Expan/2023-10-31-FOOOF-sensitivity-analysis/fooof_outputs/fooof_1-12_30-250Hz_r0_bw1_pw25'
    freq_range=[1, 250]
    
    print("fit fooof psd no beta 20231106 1017")
    psd = pd.read_table("/Volumes/Nexus/Users/busha/Analysis/2021-07-07-FOOOF-reparam/data/ITI_power_spectra.tsv",delimiter='\t')
    
    #psd.iloc[:,18:36]=6*np.random.rand(psd.shape[0],18)
    #psd.iloc[:,18:36]=np.nan
    psd=psd.drop(psd.columns[18:36],axis=1) #note that fooof/fooof/objs/fit.py line 223 self._check_freqs = False need for this to run
    ...
    spectra_array = 10**spectra.iloc[:,7:(psd.shape[1]+1)].values
    #spectra_array[:,12:30] = np.nan
